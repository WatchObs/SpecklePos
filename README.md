...
It is also possible to use a microscope lens to discern the rough surfaces of a moving element, but speckle was tried as it does not require a lens and therefore no focusing. The speckle patterns in this setup are in the near field (rather than far field) and mean speckle size is governed by local phase curvature d ≈ √( λ · z ) (rough diffuser) where λ = wavelength (m), z = distance from scattering surface to sensor - spot size plays no part as opposed to far field. Therefore the speckle increase by the square root of the distance between the speckling surface and camera sensor. The RA gear is anodized aluminum and the laser shines at 45 degrees so the speckle will be somewhat smaller.
